...
from .addressable import AddressableObject, AddressableDict, AddressableLeaf
...
class WeConnect(AddressableObject):
    ...
    def __login(self):
        ...
        while True:
            afterLoginResponse = self.__session.get(afterLogingUrl, headers=self.DEFAULT_OPTIONS['loginHeaders'], allow_redirects=False)
    ...
    def update(self):
        ...
        if data['data']:
            ...
            for vehicleDict in data['data']:
                if not 'vin' in vehicleDict:
                    break
                vin = vehicleDict['vin']
                vins.append(vin)
                if not vin in self.__vehicles:
