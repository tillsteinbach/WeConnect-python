...
class Vehicle(AddressableObject):
    ...
    def update(
        ...
        if fromDict is not None:
                ...
                for capDict in fromDict['capabilities']:
                    if 'id' in capDict:
                        if capDict['id'] in self.capabilities:
                            ...
                            self.capabilities[capDict['id']] = GenericCapability(id=capDict['id'], parent=self.capabilities, fromDict=capDict)
                for capabilityId in [capabilityId for capabilityId in self.capabilities.keys() if capabilityId not in [capability['id'] for capability in fromDict['capabilities'] if 'id' in capability]]:
            ...
            for key, value in {key: value for key, value in fromDict.items() if key not in ['vin', 'role', 'enrollmentStatus', 'model', 'nickname', 'capabilities', 'images']}.items():
    ...
    def __updateStatus(self):
        ...
        if data['data']:
            if 'accessStatus' in data['data']:
                if 'accessStatus' in self.statuses:
                    self.statuses['accessStatus'].update(fromDict=data['data']['accessStatus'])
                else:
                    self.statuses['accessStatus'] = AccessStatus(parent=self.statuses, id='accessStatus', fromDict=data['data']['accessStatus'])
            else:
                if 'accessStatus' in self.statuses:
                    del self.statuses['accessStatus']
        
            if 'batteryStatus' in data['data']:
                if 'batteryStatus' in self.statuses:
                    self.statuses['batteryStatus'].update(fromDict=data['data']['batteryStatus'])
                else:
                    self.statuses['batteryStatus'] = BatteryStatus(parent=self.statuses, id='batteryStatus', fromDict=data['data']['batteryStatus'])
            else:
                if 'batteryStatus' in self.statuses:
                    del self.statuses['batteryStatus']
            
            if 'chargingStatus' in data['data']:
                if 'chargingStatus' in self.statuses:
                    self.statuses['chargingStatus'].update(fromDict=data['data']['chargingStatus'])
                else:
                    self.statuses['chargingStatus'] = ChargingStatus(parent=self.statuses, id='chargingStatus', fromDict=data['data']['chargingStatus'])
            else:
                if 'chargingStatus' in self.statuses:
                    del self.statuses['chargingStatus']
            
            if 'chargingSettings' in data['data']:
                if 'chargingSettings' in self.statuses:
                    self.statuses['chargingSettings'].update(fromDict=data['data']['chargingSettings'])
                else:
                    self.statuses['chargingSettings'] = ChargingSettings(parent=self.statuses, id='chargingSettings', fromDict=data['data']['chargingSettings'])
            else:
                if 'chargingSettings' in self.statuses:
                    del self.statuses['chargingSettings']
            
            if 'plugStatus' in data['data']:
                if 'plugStatus' in self.statuses:
                    self.statuses['plugStatus'].update(fromDict=data['data']['plugStatus'])
                else:
                    self.statuses['plugStatus'] = PlugStatus(parent=self.statuses, id='plugStatus', fromDict=data['data']['plugStatus'])
            else:
                if 'plugStatus' in self.statuses:
                    del self.statuses['plugStatus']
            
            if 'climatisationStatus' in data['data']:
                if 'climatisationStatus' in self.statuses:
                    self.statuses['climatisationStatus'].update(fromDict=data['data']['climatisationStatus'])
                else:
                    self.statuses['climatisationStatus'] = ClimatizationStatus(parent=self.statuses, id='climatisationStatus', fromDict=data['data']['climatisationStatus'])
            else:
                if 'climatisationStatus' in self.statuses:
                    del self.statuses['climatisationStatus']
            
            if 'climatisationSettings' in data['data']:
                if 'climatisationSettings' in self.statuses:
                    self.statuses['climatisationSettings'].update(fromDict=data['data']['climatisationSettings'])
                else:
                    self.statuses['climatisationSettings'] = ClimatizationSettings(parent=self.statuses, id='climatisationSettings', fromDict=data['data']['climatisationSettings'])
            else:
                if 'climatisationSettings' in self.statuses:
                    del self.statuses['climatisationSettings']
            
            if 'windowHeatingStatus' in data['data']:
                if 'windowHeatingStatus' in self.statuses:
                    self.statuses['windowHeatingStatus'].update(fromDict=data['data']['windowHeatingStatus'])
                else:
                    self.statuses['windowHeatingStatus'] = WindowHeatingStatus(parent=self.statuses, id='windowHeatingStatus', fromDict=data['data']['windowHeatingStatus'])
            else:
                if 'windowHeatingStatus' in self.statuses:
                    del self.statuses['windowHeatingStatus']
            
            if 'lightsStatus' in data['data']:
                if 'lightsStatus' in self.statuses:
                    self.statuses['lightsStatus'].update(fromDict=data['data']['lightsStatus'])
                else:
                    self.statuses['lightsStatus'] = LightsStatus(parent=self.statuses, id='lightsStatus', fromDict=data['data']['lightsStatus'])
            else:
                if 'lightsStatus' in self.statuses:
                    del self.statuses['lightsStatus']
            
            if 'rangeStatus' in data['data']:
                if 'rangeStatus' in self.statuses:
                    self.statuses['rangeStatus'].update(fromDict=data['data']['rangeStatus'])
                else:
                    self.statuses['rangeStatus'] = RangeStatus(parent=self.statuses, id='rangeStatus', fromDict=data['data']['rangeStatus'])
            else:
                if 'rangeStatus' in self.statuses:
                    del self.statuses['rangeStatus']
            
            if 'capabilityStatus' in data['data']:
                if 'capabilityStatus' in self.statuses:
                    self.statuses['capabilityStatus'].update(fromDict=data['data']['capabilityStatus'])
                else:
                    self.statuses['capabilityStatus'] = CapabilityStatus(parent=self.statuses, id='capabilityStatus', fromDict=data['data']['capabilityStatus'])
            else:
                if 'capabilityStatus' in self.statuses:
                    del self.statuses['capabilityStatus']
            
            if 'climatisationTimer' in data['data']:
                if 'climatisationTimer' in self.statuses:
                    self.statuses['climatisationTimer'].update(fromDict=data['data']['climatisationTimer'])
                else:
                    self.statuses['climatisationTimer'] = ClimatizationTimer(parent=self.statuses, id='climatisationTimer', fromDict=data['data']['climatisationTimer'])
            else:
                if 'climatisationTimer' in self.statuses:
                    del self.statuses['climatisationTimer']
            
            for key, value in {key: value for key, value in data['data'].items() if key not in ['accessStatus', 'batteryStatus', 'chargingStatus', 'chargingSettings', 'plugStatus', 'climatisationStatus', 'climatisationSettings', 'windowHeatingStatus', 'lightsStatus', 'rangeStatus', 'capabilityStatus', 'climatisationTimer']}.items():
                logging.warning(self.getGlobalAddress()+': Unknown attribute ' + key + ' with value ' + str(value)) # TODO GenericStatus(parent=self.statuses, id=statusId, fromDict=statusDict)
        ...
        if statusResponse.status_code == 200:
            ...
            if 'data' in data:
                if 'parkingPosition' in self.statuses:
                    ...
                    self.statuses['parkingPosition'] = ParkingPosition(parent=self.statuses, id='parkingPosition', fromDict=data['data'])
            return
        if 'parkingPosition' in self.statuses:
...
class GenericCapability(AddressableObject):
    ...
    def update(self, fromDict):
        ...
        for key, value in {key: value for key, value in fromDict.items() if key not in ['id', 'status', 'expirationDate', 'userDisablingAllowed']}.items():
            logging.warning(self.getGlobalAddress()+': Unknown attribute ' + key + ' with value ' + str(value))
    
    def __str__(self):
        return f'[{self.id.value}] Status: {self.status.value} disabling: {self.userDisablingAllowed.value} (expires {self.expirationDate.value})'
...
class GenericStatus(AddressableObject):
    ...
    def update(self, fromDict, ignoreAttributes=[]):
        ...
        for key, value in {key: value for key, value in fromDict.items() if key not in (['carCapturedTimestamp'] + ignoreAttributes)}.items():
            logging.warning(self.getGlobalAddress()+': Unknown attribute ' + key + ' with value ' + str(value))
...
class AccessStatus(GenericStatus):
    ...
    def update(self, fromDict, ignoreAttributes=[]):
        ...
        if 'doors' in fromDict:
            ...
            for doorName in [doorName for doorName in self.doors.keys() if doorName not in [door['name'] for door in fromDict['doors'] if 'name' in door]]:
        ...
        if 'windows' in fromDict:
            ...
            for windowName in [windowName for windowName in self.windows.keys() if windowName not in [window['name'] for window in fromDict['windows'] if 'name' in window]]:
...
class BatteryStatus(GenericStatus):
    def __init__(
        ...
        self.cruisingRangeElectric_km = AddressableAttribute(localAddress='cruisingRangeElectric_km', value=None, parent=self)
        super().__init__(parent, id, fromDict=fromDict)
    ...
    def update(self, fromDict, ignoreAttributes=[]):
        ...
        super().update(fromDict=fromDict, ignoreAttributes=(ignoreAttributes + ['currentSOC_pct', 'cruisingRangeElectric_km']))
...
class ChargingStatus(GenericStatus):
    def __init__(
        ...
        self.remainingChargingTimeToComplete_min = AddressableAttribute(localAddress='remainingChargingTimeToComplete_min', parent=self, value=None)
    ...
    def update(self, fromDict, ignoreAttributes=[]):
        ...
        super().update(fromDict=fromDict, ignoreAttributes=(ignoreAttributes + ['remainingChargingTimeToComplete_min', 'chargingState', 'chargePower_kW', 'chargeRate_kmph']))
...
class ChargingSettings(GenericStatus):
    ...
    def update(self, fromDict, ignoreAttributes=[]):
        ...
        if 'autoUnlockPlugWhenCharged' in fromDict:
            try:
                self.autoUnlockPlugWhenCharged.value = ChargingSettings.UnlockPlugState(fromDict['autoUnlockPlugWhenCharged'])
            except ValueError:
        ...
        super().update(fromDict=fromDict, ignoreAttributes=(ignoreAttributes + ['maxChargeCurrentAC', 'autoUnlockPlugWhenCharged', 'targetSOC_pct']))
...
class PlugStatus(GenericStatus):
    ...
    def update(self, fromDict, ignoreAttributes=[]):
        ...
        super().update(fromDict=fromDict, ignoreAttributes=(ignoreAttributes + ['plugConnectionState', 'plugLockState']))
...
class ClimatizationStatus(GenericStatus):
    def __init__(
        ...
        self.remainingClimatisationTime_min = AddressableAttribute(localAddress='remainingClimatisationTime_min', parent=self, value=None)
    ...
    def update(self, fromDict, ignoreAttributes=[]):
        ...
        super().update(fromDict=fromDict, ignoreAttributes=(ignoreAttributes + ['remainingClimatisationTime_min', 'climatisationState']))
...
class ClimatizationSettings(GenericStatus):
    def __init__(
        ...
        self.climatisationWithoutExternalPower = AddressableAttribute(localAddress='climatisationWithoutExternalPower', parent=self, value=None)
    ...
    def update(self, fromDict, ignoreAttributes=[]):
        ...
        super().update(fromDict=fromDict, ignoreAttributes=(ignoreAttributes + ['targetTemperature_K', 'targetTemperature_C', 'climatisationWithoutExternalPower', 'climatizationAtUnlock', 'windowHeatingEnabled', 'zoneFrontLeftEnabled', 'zoneFrontRightEnabled', 'zoneRearLeftEnabled', 'zoneRearRightEnabled']))

    
    def __str__(self):
        string = super().__str__()+'\n'
        if self.targetTemperature_C.enabled:
            string += f'\tTarget Temperature: {self.targetTemperature_C.value} °C ({self.targetTemperature_K.value}°K) \n'
            ...
            for windowDict in fromDict['windowHeatingStatus']:
                if 'windowLocation' in windowDict:
                    if windowDict['windowLocation'] in self.windows:
                        ...
                        self.windows[windowDict['windowLocation']] = WindowHeatingStatus.Window(fromDict=windowDict, parent=self.windows)
            for windowName in [windowName for windowName in self.windows.keys() if windowName not in [window['windowLocation'] for window in fromDict['windowHeatingStatus'] if 'windowLocation' in window]]:
                del self.doors[windowName]
        else:
            self.windows.clear()
            self.windows.enabled = False
    ...
    class Window(AddressableObject):
        ...
        def update(self, fromDict):
            ...
            if 'windowHeatingState' in fromDict:
                try:
                    self.windowHeatingState.value = WindowHeatingStatus.Window.WindowHeatingState(fromDict['windowHeatingState'])
                except ValueError:
                    self.windowHeatingState.value = WindowHeatingStatus.Window.WindowHeatingState.UNKNOWN
            else:
                self.windowHeatingState.enabled = False
            
            for key, value in {key: value for key, value in fromDict.items() if key not in ['windowLocation', 'windowHeatingState']}.items():
                logging.warning(self.getGlobalAddress()+': Unknown attribute ' + key + ' with value ' + str(value))
...
class LightsStatus(GenericStatus):
    ...
    def update(self, fromDict, ignoreAttributes=[]):
        ...
        if 'lights' in fromDict:
            ...
            for lightName in [lightName for lightName in self.lights.keys() if lightName not in [light['name'] for light in fromDict['lights'] if 'name' in light]]:
...
class RangeStatus(GenericStatus):
    ...
    def update(self, fromDict, ignoreAttributes=[]):
        ...
        super().update(fromDict=fromDict, ignoreAttributes=(ignoreAttributes + ['carType', 'primaryEngine', 'secondaryEngine', 'totalRange_km']))
    ...
    class Engine(AddressableObject):
        ...
        def update(self, fromDict):
            ...
            for key, value in {key: value for key, value in fromDict.items() if key not in ['type', 'currentSOC_pct', 'remainingRange_km']}.items():
                logging.warning(self.getGlobalAddress()+': Unknown attribute ' + key + ' with value ' + str(value))
            ...
            for capDict in fromDict['capabilities']:
                if 'id' in capDict:
                    if capDict['id'] in self.capabilities:
                        ...
                        self.capabilities[capDict['id']] = GenericCapability(id=capDict['id'], fromDict=capDict, parent=self.capabilities)
            for capabilityId in [capabilityId for capabilityId in self.capabilities.keys() if capabilityId not in [capability['id'] for capability in fromDict['capabilities'] if 'id' in capability]]:
                del self.capabilities[capabilityId]
        else:
            self.capabilities.clear()
            ...
            for climatizationTimerDict in fromDict['timers']:
                if 'id' in climatizationTimerDict:
                    if climatizationTimerDict['id'] in self.timers:
                        ...
                        self.timers[climatizationTimerDict['id']] = ClimatizationTimer.Timer(fromDict=climatizationTimerDict, parent=self.timers)
            for timerId in [timerId for timerId in self.timers.keys() if timerId not in [timer['id'] for timer in fromDict['timers'] if 'id' in timer]]:
    ...
    class Timer(AddressableObject):
        ...
        def update(self, fromDict):
            ...
            if 'recurringTimer' in fromDict:
                if self.recurringTimer is None:
                    self.recurringTimer = ClimatizationTimer.Timer.RecurringTimer(localAddress='recurringTimer', parent=self, fromDict=fromDict['recurringTimer'])
                else:
            ...
            for key, value in {key: value for key, value in fromDict.items() if key not in ['id', 'enabled', 'recurringTimer']}.items():
                logging.warning(self.getGlobalAddress()+': Unknown attribute ' + key + ' with value ' + str(value))
        ...
        class RecurringTimer(AddressableObject):
            ...
            def update(self, fromDict):
                ...
                if 'recurringOn' in fromDict:
                    for day, state in fromDict['recurringOn'].items():
                        if day in self.recurringOn:
                            ...
                            self.recurringOn[day] = AddressableAttribute(localAddress=day, parent=self.recurringOn, value=state)
                ...
                for key, value in {key: value for key, value in fromDict.items() if key not in ['startTime', 'recurringOn']}.items():
                    logging.warning(self.getGlobalAddress()+': Unknown attribute ' + key + ' with value ' + str(value))
